...
- 🖼️ **图片消息** - `{{image url}}`
- 👥 **AT消息** - `{{at user_id}}`
- 🎨 **文本转图片** - `{{text_to_image "text"}}`，支持emoji自动转换
...
| **Emoji支持** | `{{text_to_image "🎉文本"}}` | `{{text_to_image "🚨告警：{{message}}"}}` |
...
🎯 状态: {{#if_equals status 'success'}}✅ 成功{{else}}❌ 失败{{/if_equals}}"}}
```

> 💡 **支持的嵌套**：`text_to_image` 支持嵌套基础变量和条件语法（`{{variable}}`、`{{#if_equals}}`）

> 🎨 **Emoji渲染**：所有emoji会自动转换为高清图片，确保在不同设备上显示一致
